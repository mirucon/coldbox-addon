...
function cd_addon_amp_embeded_tweets() {
	if ( cd_is_amp() ) {
		...
		if ( strpos( $post_content, 'twitter-tweet' ) !== false || strpos( $post_content, '<amp-twitter' ) !== false || strpos( $post_content, 'twitter.com/' ) !== false ) {
			return true;
		}
	}
}
...
function cd_addon_amp_embeded_youtube() {
	if ( cd_is_amp() ) {
		$post_content = cd_addon_amp_post_content();
		if ( preg_match( '<iframe.+?src="https:\/\/www.youtube.com\/embed\/(.+?)(\?feature=oembed)?".*?>', $post_content ) !== 0 || strpos( $post_content, '<amp-youtube' ) !== false ) {
			return true;
		}
	}
}
...
function cd_addon_amp_iframe() {
	if ( cd_is_amp() ) {
		$post_content = cd_addon_amp_post_content();
		if ( strpos( $post_content, '<iframe' ) !== false || strpos( $post_content, '<amp-iframe' ) !== false || strpos( $post_content, '[embed' ) !== false || strpos( $post_content, home_url() ) !== false ) {
			return true;
		}
	}
}
...
/**
 * Adding the AMPHTML meta tag on normal pages.
 *
 * @since 1.0.0
 */
